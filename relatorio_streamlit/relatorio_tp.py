...
df = pd.read_csv(
    "https://raw.githubusercontent.com/userhv/Forest-Fires-in-Brazil/main/database/incendiosflorestais_focoscalor_estados.csv",
    sep="\t",
)
area_estado = pd.read_csv(
    "https://raw.githubusercontent.com/userhv/Forest-Fires-in-Brazil/main/database/Area_Plantada_Estado.csv",
    sep=";",
)

# nao precisamos mais importar os dados diretamente para o streamlit (?)
def load_data(nrows):
...
fig1_title.write(
    "**Número de ocorrências de queimadas registradas no Brasil (1998-2017)**"
)
...
# Com a falta de tempo para refazer os plots nativamente no streamlit, peguei apenas as imagens geradas pelo

plot_estacoes = Image.open("./plots/queimadas_por_estacao_do_ano.png")
st.image(
    plot_estacoes,
    caption="Primavera+Inverno e Outono+Verão formam dois grupos de dados com características parecidas",
)

hist_primavera_inverno = Image.open("./plots/media_queimadas_inverno_primavera.png")
st.image(
    hist_primavera_inverno,
)
...
regressao_geral_por_meses = Image.open("./plots/regressao_geral_por_meses.png")
st.image(regressao_geral_por_meses)

regressao_fhc2 = Image.open("./plots/regressao_fhc2.png")
st.image(regressao_fhc2)
...
st.write(fig_norte)
fig_norte_title.write(
    "**Número de ocorrências de queimadas registradas na região Norte do Brasil (1998-2017)**"
)
...
fig_nordeste_title.write(
    "**Número de ocorrências de queimadas registradas na região Nordeste do Brasil (1998-2017)**"
)
...
fig_centro_oeste_title.write(
    "**Número de ocorrências de queimadas registradas na região Centro-Oeste do Brasil (1998-2017)**"
)
...
fig_sudeste_title.write(
    "**Número de ocorrências de queimadas registradas na região Sudeste do Brasil (1998-2017)**"
)
...
st.write(fig_sul)
fig_sul_title.write(
    "**Número de ocorrências de queimadas registradas na região Sul do Brasil (1998-2017)**"
)
...
# PERGUNTA 4
st.markdown(
    "## **O tipo de produto Cultivado em uma região, tem relação com o número de incêndios ocorrido nessa mesma região?**"
)

st.markdown(
    "A princípio tivemos a ideia de fazer a pergunta se havia um bioma que mais sofria com os incêndios. Após nos familiarizarmos com a base de dados e fazer algumas explorações iniciais vimos que essa pergunta tinha muito em comum com a pergunta anterior, então nos veio uma pergunta que parecia mais interessante e mais diferente: Existe uma relação entre o tipo de produto cultivado pela agricultura e o número de incêndios florestais na região?"
)
st.markdown(
    "Para fazer essa análise pegamos uma lista com os 10 principais produtos agrícolas do país, essa lista foi retirada do site: https://diarural.com.br/voce-sabe-quais-sao-os-10-principais-produtos-agricolas-no-brasil/ e com base nos dados fornecidos na página https://sidra.ibge.gov.br/tabela/1612 iremos responder a questão."
)
st.markdown(
...
# algodão
st.markdown("**Produção de algodão**")
corr_norte_algodao = Image.open("./plots/corr_norte_algodao.png")
st.image(corr_norte_algodao)

corr_nordeste_algodao = Image.open("./plots/corr_nordeste_algodao.png")
st.image(corr_nordeste_algodao)

corr_centro_oeste_algodao = Image.open("./plots/corr_centro_oeste_algodao.png")
st.image(corr_centro_oeste_algodao)

corr_sudeste_algodao = Image.open("./plots/corr_sudeste_algodao.png")
st.image(corr_sudeste_algodao)

corr_sul_algodao = Image.open("./plots/corr_sul_algodao.png")
st.image(corr_sul_algodao)

# arroz
st.markdown("**Produção de arroz**")
corr_norte_arroz = Image.open("./plots/corr_norte_arroz.png")
st.image(corr_norte_arroz)

corr_nordeste_arroz = Image.open("./plots/corr_nordeste_arroz.png")
st.image(corr_nordeste_arroz)

corr_centro_oeste_arroz = Image.open("./plots/corr_centro_oeste_arroz.png")
st.image(corr_centro_oeste_arroz)

corr_sudeste_arroz = Image.open("./plots/corr_sudeste_arroz.png")
st.image(corr_sudeste_arroz)

corr_sul_arroz = Image.open("./plots/corr_sul_arroz.png")
st.image(corr_sul_arroz)

progress_bar.progress(90)
# cana
st.markdown("**Produção de cana**")
corr_norte_cana = Image.open("./plots/corr_norte_cana.png")
st.image(corr_norte_cana)

corr_nordeste_cana = Image.open("./plots/corr_nordeste_cana.png")
st.image(corr_nordeste_cana)

corr_centro_oeste_cana = Image.open("./plots/corr_centro_oeste_cana.png")
st.image(corr_centro_oeste_cana)

corr_sudeste_cana = Image.open("./plots/corr_sudeste_cana.png")
st.image(corr_sudeste_cana)

corr_sul_cana = Image.open("./plots/corr_sul_cana.png")
st.image(corr_sul_cana)

# milho
st.markdown("**Produção de milho**")
corr_norte_milho = Image.open("./plots/corr_norte_milho.png")
st.image(corr_norte_milho)

corr_nordeste_milho = Image.open("./plots/corr_nordeste_milho.png")
st.image(corr_nordeste_milho)

corr_centro_oeste_milho = Image.open("./plots/corr_centro_oeste_milho.png")
st.image(corr_centro_oeste_milho)

corr_sudeste_milho = Image.open("./plots/corr_sudeste_milho.png")
st.image(corr_sudeste_milho)

corr_sul_milho = Image.open("./plots/corr_sul_milho.png")
st.image(corr_sul_milho)

# mandioca
st.markdown("**Produção de mandioca**")
corr_norte_mandioca = Image.open("./plots/corr_norte_mandioca.png")
st.image(corr_norte_mandioca)

corr_nordeste_mandioca = Image.open("./plots/corr_nordeste_mandioca.png")
st.image(corr_nordeste_mandioca)

corr_centro_oeste_mandioca = Image.open("./plots/corr_centro_oeste_mandioca.png")
st.image(corr_centro_oeste_mandioca)

corr_sudeste_mandioca = Image.open("./plots/corr_sudeste_mandioca.png")
st.image(corr_sudeste_mandioca)

corr_sul_mandioca = Image.open("./plots/corr_sul_mandioca.png")
st.image(corr_sul_mandioca)

# soja
st.markdown("**Produção de soja**")
corr_norte_soja = Image.open("./plots/corr_norte_soja.png")
st.image(corr_norte_soja)

corr_nordeste_soja = Image.open("./plots/corr_nordeste_soja.png")
st.image(corr_nordeste_soja)

corr_centro_oeste_soja = Image.open("./plots/corr_centro_oeste_soja.png")
st.image(corr_centro_oeste_soja)

corr_sudeste_soja = Image.open("./plots/corr_sudeste_soja.png")
st.image(corr_sudeste_soja)

corr_sul_soja = Image.open("./plots/corr_sul_soja.png")
st.image(corr_sul_soja)

st.markdown(
    "Podemos notar algo curioso com essa análise, de fato, pra grande maioria dos produtos podemos ver pouca correlação entre o número de focos e a produção, porém três casos merecem serem notados. Para os produtos Cana, Mandioca e Milho na região Centro-Oeste há uma correlação positiva considerável. Ou seja, a medida que a produção em quilo por hectare de cana e mandioca aumenta, o número de focos de incêndio na região aumenta também."
)
